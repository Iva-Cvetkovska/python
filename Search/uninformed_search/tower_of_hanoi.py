...
def valid_move(current_tower, next_tower):
    if len(next_tower) == 0:
        return True

    element_to_move = current_tower[-1]
    element_to_compare = next_tower[-1]
...
class TowerOfHanoi(Problem):
    def __init__(self, num_of_towers, initial, goal):
        super().__init__(initial, goal)
        self.num_of_towers = num_of_towers

    def successor(self, state):
        successors = {}

        for i in range(self.num_of_towers):
            current_tower = state[i]  # (3, 2, 1)
            if len(current_tower) == 0:
                continue
            for j in range(self.num_of_towers):
                if i != j:
                    next_tower = state[j]  # ()
                    if valid_move(current_tower, next_tower):
                        new_curr_tower = list(current_tower)
                        block = new_curr_tower.pop()

                        new_next_tower = list(next_tower)
                        new_next_tower.append(block)

                        new_state = list(state)
                        new_state[i] = tuple(new_curr_tower)
                        new_state[j] = tuple(new_next_tower)
...
if __name__ == "__main__":
    blocks = input().split(';')  # 3,2,1;;
    towers = [tuple(map(int, block.split(','))) if block else () for block in blocks]

    goal_blocks = input().split(';')  # ;;3,2,1
    goal_towers = [tuple(map(int, block.split(','))) if block else () for block in goal_blocks]

    tower_of_hanoi = TowerOfHanoi(len(towers), tuple(towers), tuple(goal_towers))

    result = breadth_first_graph_search(tower_of_hanoi)
